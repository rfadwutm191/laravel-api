...
class PembelianController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        $request->validate([
            ...
            'items.*.new_produk.expiredDate' => 'nullable|date',
        ]);
        ...
        try {
            ...
            foreach ($request->items as $item) {
                ...
                if (
                    !$produkId &&
                    !empty($item['new_produk']['nama_barang'])
                ) {
                    ...
                    $produkBaru = Produk::create([
                        'kodeBarang' => $np['kode_barang'],
                        'namaBarang' => $np['nama_barang'],
                        'kategori' => $np['kategori'] ?? null,
                        'harga' => $np['harga'] ?? $item['harga_beli'],
                        'stok' => $np['stok'] ?? 0,
                        'expiredDate' => $np['expiredDate'] ?? null,
                        'rating' => 0,
                    ]);
            ...
            foreach ($itemsData as $itemData) {
                ...
                $pembelian->items()->create($itemData);
            }
    ...
    public function updateStatus(
        ...
        if ($pembelian->status === 'diterima') {
            ...
            return response()->json([
                'message' =>
                    'Pembelian yang sudah diterima tidak dapat diubah statusnya'
            ], 422);
        }
